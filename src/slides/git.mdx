...
第一个问题是我们为什么要用版本控制工具。（等回答）

版本控制工具会记录我们代码版本变化的历史。
...
对应到记录中就是：用户信息、commit message、代码

好的记录信息，可以快速定位我们需要的版本。
...
- username 用你最广为人知的名字，比如中文名。
- email 用公司邮箱。

[Git 如何配置多用户](https://confluence.fiture.com/pages/viewpage.action?pageId=36443964)

<Notes>

用户信息，要做到，一眼就能看出来是谁改的。
...
commit message，要做到，一眼就能看出来改了啥，有什么影响。
...
Git 有很多特性，这些特性决定了它的优缺点。我认为比较重要的有以下两个：

分布式让提交操作更顺畅。（提交操作，不需要联网，不受网络限制）

快照机制让分支操作更顺畅。（只是增加一个指针，成本很低）
...
**仓库之间的交互** 由 **分支之间的交互** 组成

而 **分支之间的交互** 由 **文件之间的交互** 组成

## The End!

以上就是我对 Git 的理解（本次分享的主要内容）

因为能力有限、时间也有限，难免有理解不对或者疏漏的地方，还望大家批评指正。
...
- 从根本上来讲 Git 是一个内容寻址文件系统。不用 commit，可以保存文件。
- gc：并非严格地基于快照，也会基于差异去记录历史版本。
- 远程仓库的镜像与本地仓库，不同的分支，它们在存储上并没有严格的边界
- reflog：删除了的东西并不会立刻删除，是可以找回的。
- etc.
...
- [Git 协作规范 v1.0](https://confluence.fiture.com/pages/viewpage.action?pageId=29635955)
- [Post-commit Code Review 指南 - v0.9 - RFC](https://confluence.fiture.com/pages/viewpage.action?pageId=36443938)
